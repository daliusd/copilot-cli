...
async function runPrompt(options) {
  ...
  const payload = {
    ...
    tools: options.tools?.map((name) => TOOLS_MAP[name].tool),
  };
    ...
    try {
      ...
      response.data.on('data', (chunk) => {
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          if (!jsonBuffer && line.startsWith('data: ')) {
            if (line === 'data: [DONE]') {
              continue;
            }
